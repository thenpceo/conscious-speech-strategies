...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    async function exchangeToken() {
      ...
      // 2. Try to extract token from URL hash (Supabase recovery links use hash fragments)
      ...
      if (hash) {
        ...
        if (accessToken && refreshToken) {
          ...
          if (sessionError) {
            console.error("[reset-password] Token exchange failed:", sessionError.message);
          }
        }
      }
      ...
      if (token && type) {
        ...
        if (verifyError) {
          console.error("[reset-password] OTP verification failed:", verifyError.message);
        }
      }

      // 4. Listen for auth state changes (PASSWORD_RECOVERY event)
      ...
      <div className="w-full max-w-[420px]">
        ...
        {!hasSession && (
          <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-sm">
            <strong>Reset link expired or invalid.</strong> Please go back to the login page and request a new one.
          </div>
        )}

        <form onSubmit={handleResetPassword} className="bg-white rounded-2xl shadow-sm shadow-slate-200/50 border border-slate-200/60 p-7 space-y-5">
          <div>
        ...
        <div className="text-center mt-5">
          <a href="/admin/login" className="text-sm text-teal-600 hover:text-teal-700 transition-colors">
            Back to sign in
          </a>
        </div>

        <p className="text-center text-xs text-slate-400 mt-4">Conscious Speech Strategies Admin Portal</p>
      </div>
    </div>
  );
}
